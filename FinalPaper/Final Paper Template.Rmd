...
p1 + p4 + p2 + p3 + plot_layout(ncol = 2, nrow = 2, axis_titles = "collect") + plot_annotation("Statistics grouped by Tier", caption = "")
```

|      This figure shows how generally the tiers are correlated with characters that perform better in the game. It does this by comparing the 4 continuous variables of KDA, WinRate, PickRate, and BanRate to each tier. Higher values for all of these statistics indicate that a character is better. This does show that generally the higher tiers have better champions however, there do appear to be discrepancies in the KDA and WinRate figures. This is because it shows examples of worse tiers performing better in their given metric. This is precisely the goal of our analysis to evaluate and perhaps create better tiers for characters.
...
|      For the second question, we want to understand how our model performs based on temporary and non-temporary champions. We define a temporary champion to be any champion who is not present in a specific role for all 23 patches (`df %>% group_by(Name, Role) %>% tally() %>% filter(n < 23)`). For example, Ivern is present in the Jungle role for 23 patches, so these observations would be considered non-temporary, but Ivern is present in the Top role for 22 patches, so only these 22 patches would be considered temporary.
...
|      We want to investigate this question because temporary champions are one of the main reasons for tier lists to exist, meta champions that have high pick ban rates and sporadic win rates are often evaluated by casual players in order to understand whether or not they are viable. This question focuses on the possible difference between these two groups, is there a significant difference in the way that temporary champions are classified that would matter to a potential player. From the figure above, it's evident that there is no clear grouping for these champions, they are distributed randomly, which makes it an excellent test group because we are not testing just one small section of our model (for example champions with win rates over $52$%). 


### Feature Selection

|      The feature space should first be reduced as the original dimensions of the dataset (around 200) are too high for a meaningful conclusion from reasonable computational power to be drawn from. To do this, this paper employs a two-step technique: A stepwise regression to determine the optimal number of features for each tier, and a Boruta model to determine which features are most important to the dataset for all six tiers. 
...
|      From our stepwise regression, we can see that 8 features are likely to be optimal since any extra features beyond this point result in minimal payoffs. 
...
|      Using our target feature count from the stepwise regression, we turn to a Boruta model. This technique creates ‘shadow variables’ which are randomly sorted copies of the original vectors in the data frame, and then a `RandomForest` model is trained on the dataset with shadow features added. From this, it looks to see the importance of each feature (which is calculated with complex mathematical formulas such as Impurity) and compares the most important shadow feature with the importance of all the original features. In this way, we see whether or not any variable performs more effectively than essential randomness, which can help to determine how useful it will be in the end. Our Boruta model tells us that `WinRate`, `PickRate`, `Role`, and `BanRate` are the four most important variables, but since `Role` accounts for 5 features (Top, Jungle, Mid, ADC, Support) we have $8$ total features accounted for within these $4$ variables.
...
|      We use three types of models in our analysis, two Support Vector Machines (SVMs), and one Ranger model. An SVM model works by partitioning the feature spaces into disjoint subsets. A useful metaphor is taking your house and dividing it into separate rooms, you have partitioned a feature space (your house) into disjoint subsets (rooms). We include two SVM models, one with a radial kernel and one with a linear kernel. Both models create hyper planes, which can be thought of as lines in the feature space. A linear kernel would create a linear line, for example $f(x) = x$. This model performs best and should be preferred when the data is linearly separable. When data isn't linearly separable, an often used choice is the radial kernel, which use a more complicated exponential expression to create a linear data set between the data by shifting and stretching the points, before creating a linear map that separates the data. While this shift is important, when visualizing this model the focus is on the decision boundaries, which is the projected hyper plane between data points, which will appear in the original data as non-linear. Finally, a ranger model with is a fast implementation of a random forest specializing in high dimensional data. While this data-set is not high-dimensional, it is also also proven to scale best with the number of observations and trees to fit, both of which are high for our data set.
...
|       "Ctrl -" to zoom out for the Decision Boundary Contour Plot
...
|      Something we could do better is to implement noise injections to prevent overtraining or overfitting in our model. Noise injection is when a randomly generated vector (error/noise) is added to a column/variable so that the model does not pick up particular random patterns in a data set, such that the model fits too well to that data set and does worse at predictions for other observations. In terms of additional data, there could potentially be value in a "rank" variable indicative of how good players generally are playing a said champion within a given role. Because of disparate skill floors and ceilings, champions may perform differently in different ranks. Mechanically-intensive champions may require a lot more game knowledge to pull off, whereas simple champions can achieve relative success easily. 
