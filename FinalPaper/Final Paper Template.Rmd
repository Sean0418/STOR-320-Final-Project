...
|   The difference between the two accuracy's is large, at `r round(diff, 3)`. To test for statistical significance, we use a Welsh's two-sample T test as described in the cross validation subsection. The T-statistic is `r round(t, 3)`, which corresponds to a P-value of $4 \cdot 10^{-6}$, and is statistically significant at the $0.01$ level. Therefore, we fail to reject the reject the null hypothesis, and conclude that our model performs worse on temporary champions compared to nontemporary champions.
