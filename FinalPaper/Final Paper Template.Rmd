...
crossdf <- data.frame(sample = 1:20, svm_linear = 0, svm_radial = 0, ranger = 0)
...
for (i in 1:20) {
  ...
  crossdf[i, 2] <- trainandtest("svmLinear", traindf, testdf, fitControl)
  crossdf[i, 3] <- trainandtest("svmRadial", traindf, testdf, fitControl)
  crossdf[i, 4] <- trainandtest("ranger", traindf, testdf, fitControl)
}
```
```{r}
colMeans(crossdf)
...
#K-Fold or LOOCV? Probably K-Fold bc LOOCV Is slow ash.
leaguedf$sample <- floor(runif(nrow(leagueTest), min = 1, max = 10))
svmlin_params = svmlin_fit$bestTune[1, ]
svmrad_params = svmrad_fit$bestTune[1, ]
rf_params = rf_fit$bestTune[1, ]

leaguedf <- leaguedf %>%
  mutate(svmlin_pred = 0, svmrad_pred = 0, ranger_pred = 0)
for (i in 1:10) {
  sample <- leaguedf %>%
    filter(!(sample == i))
  svmlin_fit <- train(Tier ~ .,
  ...
  leaguedf <- leaguedf %>%
    mutate(svmlin_pred = ifelse(sample == i, predict(svmlin_fit, .), svmlin_pred))
