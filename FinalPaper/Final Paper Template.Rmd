...
for (i in 1:6) {
  open3d()
  plot3d(PlottingData[-4], col= PlottingData$Tier)
  surface = computeContour3d(vol = contour.df, maxvol = max(contour.df), level = i, contour.list$WinRate, contour.list$PickRate, contour.list$BanRate) #LEVEL CONTROLS WHAT TIER WE WANT TO VIEW
  triangles3d(x = surface[,1], y = surface[,2], z = surface[,3], color = i)
