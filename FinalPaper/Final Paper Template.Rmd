...
```{r PbrCorrelation, echo  = F, warning = F}
...
Volatility Overview
```{r Std winRate, echo = F, message = F}
leaguedf %>% 
  group_by(Name) %>% 
...
```{r, message = F, echo = F, eval = F, echo = F}
Propdf <- leaguedf %>%
...
pw <- (p1 + p2) / (p3 + p4) + plot_layout(guides = "collect", axis_titles = "collect") + plot_annotation(title = "Models and Mean Cross-Validated Accuracy")
...
tab <- bestModels %>% formattable(
  list(`WinRate` = formatter("span",
                        x ~ icontext(ifelse(x > 0, "ok", "remove"), ifelse(x > 0, "", "")),
...
bestModels %>% slice_max(Accuracy, n=6) %>%
  mutate(meanDiff = max(Accuracy) - Accuracy, 
         ...
         pval = round(pnorm(q=z, lower.tail=F), 4)) %>%
  rename(`Z Score` = z,`Distance from $\\mu_0$` =  meanDiff, `P Value` = pval) %>%
