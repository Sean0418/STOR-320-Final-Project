...
```{r}
# TODO: Clean Leaguedf to only have 20 entries for each tier. Randomly chosen to make plot cleaner
leaguedf %>%
  ggplot(mapping=aes(x=winr, y=pbr, fill= Tier)) + geom_raster(data=Boundarydf, alpha=0.5) + geom_point(shape=21, mapping=aes(size=KDA), alpha=0.75) + theme_minimal() + xlab('Win Rate') + ylab('Pick Ban Rate') + ggtitle('Boundary Plot for RF Model Pre-Hypertuning')
