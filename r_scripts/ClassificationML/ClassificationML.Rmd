...
- TODO: 
  ...
  - Chose not to discretize winr/pbr for the reason that it is more useful for <= than it is for whatever else.
```{r}
#This first part may be unneeded if you can tell discretizeDF to not like discretize everything Also currently not in use as discretize messed up the accuracy, see later for better preprocess techniques! <3
tempdisc <- leaguedf %>% select(c(4,8)) %>%
  discretizeDF(methods = list(
    pbr = list(method = "interval", breaks = 100),
    winr = list(method = "interval", breaks= 100)
  ), default = list(methods="None"))
...
grid <- expand.grid(mtry = c(6,9,12,15), splitrule=c("extratrees", "gini"), min.node.size=c(1,3,6,10))
...
finalBoundarydf = expand.grid(winr = seq(0.4, 0.6, length.out=50), pbr = seq(0, 1, length.out=50), KDA = seq(1, 5, length.out=40), Role = "SUPPORT", `Role %` = seq(0.1,1, length.out=10), Patch=c(12), Class=c("Mage", "Support"))
finalBoundarydf$Tier <- predict(fit, Boundarydf)
