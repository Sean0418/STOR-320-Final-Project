...
### Preprocessing Data
- TODO: 
  - Noise injection into data and replication of data using noise to expand training set.
```{r}
...
leaguedf_noise <- rbind(leagueTrain, leagueTrain, leagueTrain, leagueTrain, leagueTrain) #This way we have 3x as much data, but we have 3 entries for every unique row
corrupt <- rbinom(length(leaguedf_noise$Tier), 1, 0.5) #Choose about 50 Percent of the data at random to corrupt with noise injection
corrupt <- as.logical(corrupt)

noise <- rnorm(sum(corrupt), 0, 0.01)

leaguedf_noise$KDA[corrupt] = leaguedf_noise$KDA[corrupt] + noise
leaguedf_noise$pbr[corrupt] = leaguedf_noise$pbr[corrupt] + noise
leaguedf_noise$winr[corrupt] = leaguedf_noise$winr[corrupt] + noise
