...
ImpFrame <- ImpFrame$importance %>% as_tibble(rownames="VariableName") %>%
  mutate(VariableName = case_when(
    "Class" %in% VariableName ~ "Class",
    "Role" %in% VariableName ~ "Role",
    TRUE ~ VariableName
  )) # Try double ifelse
ImpFrame
...
finalBoundarydf = expand.grid(winr = seq(0.4, 0.6, length.out=50), 
                            ...
                            Role = "SUPPORT", 
                            `Role %` = seq(0.1,1, length.out=10), 
                            Patch=c(12), 
                            Class = c("Mage", "Support"))


finalBoundarydf$Tier <- predict(fit, Boundarydf) #Why does it say Class not found?
...
- Null Hypothesis: The Pick Ban Rate is not an important indicator in the classification algorithm, the difference between a complete alogrithm and an alogrithm without PBR is not statistically significant in the negative direction.

```{r}
pbrOnly <- leaguedf %>%
  select(-'pbr') %>%
  mutate(Patch = as.numeric(str_replace(Patch, '(.*?)_(.*?)', '')))
pbrTrain_rows <- createDataPartition(y=leaguedf_final$Tier, p=0.75, list=FALSE) 

pbrTrain <- pbrOnly[-pbrTrain_rows, ]
pbrTest <- pbrOnly[pbrTrain_rows, ]

pbrFit <- train(as.factor(Tier)~.,
             data=pbrTrain,
             method="ranger",
)
```
...
```{r}
paste("Initial accuracy is: ", round(sum(pbrTest$Tier == predict(pbrFit, pbrTest)) / length(pbrTest$Tier), 4)*100, '%', sep='')
```

```{r}
#Hypertuning Model before finishing
grid <- expand.grid(mtry = c(6,9,12, 14), splitrule=c("extratrees", "gini"), min.node.size=c(1,3,6,10))

fitControl <- trainControl(method='CV', number = 5, verboseIter=FALSE)

pbrTunedFit <- train(as.factor(Tier)~.,
             data=pbrTrain,
             method="ranger",
             tuneGrid=grid,
             trControl = fitControl, importance="impurity"
)
print(pbrTunedFit$finalModel)
```

```{r}
paste("Initial accuracy is: ", round(sum(pbrTest$Tier == predict(pbrFit, pbrTest)) / length(pbrTest$Tier), 4)*100, '%', sep='')
paste("Final accuracy is: ", round(sum(pbrTest$Tier == predict(pbrTunedFit, pbrTest)) / length(pbrTest$Tier), 4)*100, '%', sep='')

```

```{r}
#TODO: significance testing, probably bayesian
