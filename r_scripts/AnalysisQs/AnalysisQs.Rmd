...
```{r, warning = F, fig.height = 20, fig.width = 10}
...
RoleByClass <- leaguedf %>%
  ...
  summarize(Percent = sum(PlayRate), WinRate = mean(WinRate)) %>%
  ungroup()
...
plot1a <- RoleByClass %>% 
  ggplot() + 
  geom_boxplot(mapping = aes(x = Role, color = Class, y = Percent)) + 
...
plot1a + plot2a + plot_annotation(title = "Boxplots analyzing Role and Class relationships with pickrate",
